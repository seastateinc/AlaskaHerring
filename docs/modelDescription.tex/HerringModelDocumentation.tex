  ...
  \begin{table}[h]
    ...
    \begin{align}
      ...
      & \iota_j = \begin{cases}
          ...
          \end{cases} \\[1ex]
      & N_{i,j} = \ddot{R} \exp(\delta_{i-j}) \iota_j \qquad \qquad  i=1980,\forall j \\
      & N_{i,j} = \bar{R} \exp(\delta_i) \qquad  \quad \qquad \quad \forall i, j=3 \\
      %%
      & \mbox{Dynamic States ($i > 1980$)} \nonumber\\
      & Q_{i,j} = \frac{N_{i,j} S_{i,j}}{\sum_j N_{i,j} S_{i,j}} \\
      & \bar{w}_i  = \sum_j w_j  Q_{i,j} \\
      & C_{i,j} = \frac{\hat{c}_{i}Q_{i,j}}{\bar{w}_i} \qquad \mbox{where $\hat{c}_i$ is the observed catch (mt)} \\
